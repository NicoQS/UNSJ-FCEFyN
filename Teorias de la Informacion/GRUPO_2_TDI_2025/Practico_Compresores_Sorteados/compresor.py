...
class BitReader:
    ...
    Complemento de BitWriter, permite leer datos que fueron escritos
    a nivel de bits.
    
    Funcionamiento:
        - Lee bytes del buffer uno a la vez
        - Extrae bits individuales del byte actual
        - Avanza al siguiente byte cuando se agotan los bits del actual
    
    Attributes:
        data (bytes): Datos de entrada
        pos (int): Posición actual en los datos (índice de byte)
        bit_pos (int): Posición del bit dentro del byte actual (0-7)
        curr (int): Byte actual siendo leído
    
    Ejemplo de uso:
        reader = BitReader(data)
        bit1 = reader.read_bit()
        value = reader.read_bits(3)  # Lee 3 bits
    """
    def __init__(self, data: bytes):
        self.data = data  # Datos de entrada
        self.pos = 0  # Posición en el buffer
        self.bit_pos = 8  # Forzar carga del primer byte
        self.curr = 0  # Byte actual

    def _ensure_byte(self):
        """
        Asegura que hay un byte disponible para leer.
        
        Si ya se leyeron todos los bits del byte actual, carga el siguiente
        byte del buffer.
        
        Raises:
            EOFError: Si no hay más datos disponibles
        """
        if self.bit_pos == 8:
            # Necesitamos cargar un nuevo byte
    ...
    def read_bit(self):
        ...
        """
        self._ensure_byte()
        # Extraer bit desde la izquierda (más significativo primero)
    ...
    def read_bits(self, n):
        """
        Lee múltiples bits y los retorna como un entero.
        
        Args:
            n (int): Número de bits a leer
        
        Returns:
            int: Valor leído (de n bits)
        
        Raises:
            EOFError: Si no hay suficientes datos disponibles
        """
        v = 0
        # Leer bits uno por uno y construir el valor
        for _ in range(n):
            v = (v << 1) | self.read_bit()
        return v
